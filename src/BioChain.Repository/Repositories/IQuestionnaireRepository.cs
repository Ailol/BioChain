...
public interface IQuestionnaireRepository
{
    ...
    Task<List<QuestionnaireEntity>> GetByPersonAsync(Guid subjectId, CancellationToken ct = default);
    ...
    Task AddAnswersAsync(IEnumerable<QuestionnaireAnswerEntity> answers, CancellationToken ct = default);

    // Items by sort order (for building analysis text)
    Task<List<QuestionnaireItemEntity>> GetItemsBySortOrdersAsync(IEnumerable<int> sortOrders, CancellationToken ct = default);
}
