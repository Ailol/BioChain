...
public class QuestionnaireRepository(BioChainDbContext db) : IQuestionnaireRepository
{
    ...
    public Task<List<QuestionnaireEntity>> GetByPersonAsync(Guid subjectId, CancellationToken ct = default)
        => db.Questionnaires.Where(q => q.SubjectId == subjectId).OrderByDescending(q => q.CreatedAt).ToListAsync(ct);
    ...
    public async Task AddAnswersAsync(IEnumerable<QuestionnaireAnswerEntity> answers, CancellationToken ct = default)
    {
        db.QuestionnaireAnswers.AddRange(answers);
        await db.SaveChangesAsync(ct);
    }

    public Task<List<QuestionnaireItemEntity>> GetItemsBySortOrdersAsync(
        IEnumerable<int> sortOrders, CancellationToken ct = default)
    {
        var set = sortOrders.ToHashSet();
        return db.QuestionnaireItems
            .Where(i => set.Contains(i.SortOrder))
