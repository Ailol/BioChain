...
namespace SpacetimeDB.Types
{
    ...
    public sealed partial class RemoteTables : RemoteTablesBase
    {
        ...
        {
            AddTable(Conv = new(conn));
            AddTable(DeltaLog = new(conn));
            AddTable(DeltaOp = new(conn));
            AddTable(Diag = new(conn));
            AddTable(DiffResult = new(conn));
            AddTable(Edge = new(conn));
            AddTable(MetaOp = new(conn));
            AddTable(Node = new(conn));
            AddTable(Program = new(conn));
            AddTable(SimRun = new(conn));
            AddTable(SimTick = new(conn));
            AddTable(Snapshot = new(conn));
            AddTable(SnapshotEdge = new(conn));
            AddTable(SnapshotNode = new(conn));
            AddTable(TauAcc = new(conn));
            AddTable(Tensor = new(conn));
        }
    }
    ...
    public sealed class QueryBuilder
    {
        ...
        {
            new QueryBuilder().From.Conv().ToSql(),
            new QueryBuilder().From.DeltaLog().ToSql(),
            new QueryBuilder().From.DeltaOp().ToSql(),
            new QueryBuilder().From.Diag().ToSql(),
            new QueryBuilder().From.DiffResult().ToSql(),
            new QueryBuilder().From.Edge().ToSql(),
            new QueryBuilder().From.MetaOp().ToSql(),
            new QueryBuilder().From.Node().ToSql(),
            new QueryBuilder().From.Program().ToSql(),
            new QueryBuilder().From.SimRun().ToSql(),
            new QueryBuilder().From.SimTick().ToSql(),
            new QueryBuilder().From.Snapshot().ToSql(),
            new QueryBuilder().From.SnapshotEdge().ToSql(),
            new QueryBuilder().From.SnapshotNode().ToSql(),
            new QueryBuilder().From.TauAcc().ToSql(),
            new QueryBuilder().From.Tensor().ToSql(),
        }
        ;
    }
    ...
    {
        public global::SpacetimeDB.Table<Conv, ConvCols, ConvIxCols> Conv() => new("conv", new ConvCols("conv"), new ConvIxCols("conv"));
        public global::SpacetimeDB.Table<DeltaLog, DeltaLogCols, DeltaLogIxCols> DeltaLog() => new("delta_log", new DeltaLogCols("delta_log"), new DeltaLogIxCols("delta_log"));
        public global::SpacetimeDB.Table<DeltaOp, DeltaOpCols, DeltaOpIxCols> DeltaOp() => new("delta_op", new DeltaOpCols("delta_op"), new DeltaOpIxCols("delta_op"));
        public global::SpacetimeDB.Table<Diag, DiagCols, DiagIxCols> Diag() => new("diag", new DiagCols("diag"), new DiagIxCols("diag"));
        public global::SpacetimeDB.Table<DiffResult, DiffResultCols, DiffResultIxCols> DiffResult() => new("diff_result", new DiffResultCols("diff_result"), new DiffResultIxCols("diff_result"));
        public global::SpacetimeDB.Table<Edge, EdgeCols, EdgeIxCols> Edge() => new("edge", new EdgeCols("edge"), new EdgeIxCols("edge"));
        public global::SpacetimeDB.Table<MetaOp, MetaOpCols, MetaOpIxCols> MetaOp() => new("meta_op", new MetaOpCols("meta_op"), new MetaOpIxCols("meta_op"));
        public global::SpacetimeDB.Table<Node, NodeCols, NodeIxCols> Node() => new("node", new NodeCols("node"), new NodeIxCols("node"));
        public global::SpacetimeDB.Table<Program, ProgramCols, ProgramIxCols> Program() => new("program", new ProgramCols("program"), new ProgramIxCols("program"));
        public global::SpacetimeDB.Table<SimRun, SimRunCols, SimRunIxCols> SimRun() => new("sim_run", new SimRunCols("sim_run"), new SimRunIxCols("sim_run"));
        public global::SpacetimeDB.Table<SimTick, SimTickCols, SimTickIxCols> SimTick() => new("sim_tick", new SimTickCols("sim_tick"), new SimTickIxCols("sim_tick"));
        public global::SpacetimeDB.Table<Snapshot, SnapshotCols, SnapshotIxCols> Snapshot() => new("snapshot", new SnapshotCols("snapshot"), new SnapshotIxCols("snapshot"));
        public global::SpacetimeDB.Table<SnapshotEdge, SnapshotEdgeCols, SnapshotEdgeIxCols> SnapshotEdge() => new("snapshot_edge", new SnapshotEdgeCols("snapshot_edge"), new SnapshotEdgeIxCols("snapshot_edge"));
        public global::SpacetimeDB.Table<SnapshotNode, SnapshotNodeCols, SnapshotNodeIxCols> SnapshotNode() => new("snapshot_node", new SnapshotNodeCols("snapshot_node"), new SnapshotNodeIxCols("snapshot_node"));
        public global::SpacetimeDB.Table<TauAccumulator, TauAccCols, TauAccIxCols> TauAcc() => new("tau_acc", new TauAccCols("tau_acc"), new TauAccIxCols("tau_acc"));
        public global::SpacetimeDB.Table<Tensor, TensorCols, TensorIxCols> Tensor() => new("tensor", new TensorCols("tensor"), new TensorIxCols("tensor"));
    }
    ...
    public sealed class DbConnection : DbConnectionBase<DbConnection, RemoteTables, Reducer>
    {
        ...
        protected override bool Dispatch(IReducerEventContext context, Reducer reducer)
        {
            ...
            {
                Reducer.AddConv args => Reducers.InvokeAddConv(eventContext, args),
                Reducer.AddDelta args => Reducers.InvokeAddDelta(eventContext, args),
                Reducer.AddDiag args => Reducers.InvokeAddDiag(eventContext, args),
                Reducer.AddEdge args => Reducers.InvokeAddEdge(eventContext, args),
                Reducer.AddMeta args => Reducers.InvokeAddMeta(eventContext, args),
                Reducer.AddNode args => Reducers.InvokeAddNode(eventContext, args),
                Reducer.AddTensor args => Reducers.InvokeAddTensor(eventContext, args),
                Reducer.ComputeConvergence args => Reducers.InvokeComputeConvergence(eventContext, args),
                Reducer.CreateProgram args => Reducers.InvokeCreateProgram(eventContext, args),
                Reducer.DiffSnapshots args => Reducers.InvokeDiffSnapshots(eventContext, args),
                Reducer.GetEvidenceTrace args => Reducers.InvokeGetEvidenceTrace(eventContext, args),
                Reducer.IngestBnf args => Reducers.InvokeIngestBnf(eventContext, args),
                Reducer.LintBnf args => Reducers.InvokeLintBnf(eventContext, args),
                Reducer.LogDeltaFired args => Reducers.InvokeLogDeltaFired(eventContext, args),
                Reducer.QueryConv args => Reducers.InvokeQueryConv(eventContext, args),
                Reducer.QueryDeltaLog args => Reducers.InvokeQueryDeltaLog(eventContext, args),
                Reducer.QueryDeltas args => Reducers.InvokeQueryDeltas(eventContext, args),
                Reducer.QueryDiffs args => Reducers.InvokeQueryDiffs(eventContext, args),
                Reducer.QueryEdgesFrom args => Reducers.InvokeQueryEdgesFrom(eventContext, args),
                Reducer.QueryGraph args => Reducers.InvokeQueryGraph(eventContext, args),
                Reducer.QueryMeta args => Reducers.InvokeQueryMeta(eventContext, args),
                Reducer.QueryNode args => Reducers.InvokeQueryNode(eventContext, args),
                Reducer.QuerySimRuns args => Reducers.InvokeQuerySimRuns(eventContext, args),
                Reducer.QuerySimTrace args => Reducers.InvokeQuerySimTrace(eventContext, args),
                Reducer.QuerySnapshots args => Reducers.InvokeQuerySnapshots(eventContext, args),
                Reducer.QueryTensors args => Reducers.InvokeQueryTensors(eventContext, args),
                Reducer.Reconstruct args => Reducers.InvokeReconstruct(eventContext, args),
                Reducer.Simulate args => Reducers.InvokeSimulate(eventContext, args),
                Reducer.StoreRawBnf args => Reducers.InvokeStoreRawBnf(eventContext, args),
                Reducer.TakeSnapshot args => Reducers.InvokeTakeSnapshot(eventContext, args),
                Reducer.Validate args => Reducers.InvokeValidate(eventContext, args),
