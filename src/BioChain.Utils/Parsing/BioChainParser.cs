...
public static partial class BioChainParser
{
    ...
    // ── Formula signal refs: DA@VTA → ... → GLU@PFC  or  DA[↑↑] @VTA  or  DA[↑↑] ──

    public static ((string Code, string? Region)? Source, (string Code, string? Region)? Target)
        ExtractFormulaSignalRefs(string formula)
    {
        // Try strict pattern first (requires @REGION)
        var matches = SignalRefPattern().Matches(formula);
        if (matches.Count > 0)
        {
            static (string, string?) ExtractStrict(Match m) =>
                (m.Groups["code"].Value, m.Groups["region"].Success ? m.Groups["region"].Value : null);

            var src = ExtractStrict(matches[0]);
            var tgt = matches.Count > 1 ? ExtractStrict(matches[^1]) : src;
            return (src, tgt);
        }

        // Fallback: match CODE[state] without requiring @REGION
        var fallback = SignalRefFallbackPattern().Matches(formula);
        if (fallback.Count == 0) return (null, null);

        static (string, string?) ExtractFallback(Match m) =>
            (m.Groups["code"].Value, m.Groups["region"].Success && m.Groups["region"].Value.Length > 0
                ? m.Groups["region"].Value : null);

        var fbSrc = ExtractFallback(fallback[0]);
        var fbTgt = fallback.Count > 1 ? ExtractFallback(fallback[^1]) : fbSrc;
        return (fbSrc, fbTgt);
    }
    ...
    /// <summary>
    /// Extracts a signal code and optional region from a limiter reaction string.
    /// Examples: "DA.synthesis @VTA" → ("DA", "VTA"), "5HT.release" → ("5HT", null),
    /// "eCB.ANA[↓] @PFC" → ("ANA", "PFC"), "GABA.synthesis @PFC" → ("GABA", "PFC").
    /// </summary>
    public static (string Code, string? Region)? ExtractReactionSignalRef(string reaction)
    {
        if (string.IsNullOrWhiteSpace(reaction)) return null;

        var m = ReactionSignalRefPattern().Match(reaction);
        if (!m.Success) return null;

        // If there's a sub-code (e.g., "eCB.ANA"), use the sub-code as the actual signal
        var code = m.Groups["subcode"].Success && m.Groups["subcode"].Value.Length > 0
            ? m.Groups["subcode"].Value
            : m.Groups["code"].Value;

        var region = m.Groups["region"].Success && m.Groups["region"].Value.Length > 0
    ...
    // Signal refs in formulas (strict): CODE@REGION or CODE[state] @REGION
    // Matches: DA@VTA, GLU@PFC, DA[↑↑] @NAc
    [GeneratedRegex(@"(?<code>\w+)(?:@|\[.+?\]\s*@)(?<region>\w+)")]
    private static partial Regex SignalRefPattern();

    // Signal refs in formulas (fallback): CODE[state] with optional @REGION
    // Matches: CURIOSITY[↑], DMN[↑], NE[≈], DA[↑↑] @VTA
    // Skips DEF := prefix by anchoring after common operators/delimiters
    [GeneratedRegex(@"(?<code>[A-Z][A-Z0-9_]*)\[[^\]]+\](?:\s*@(?<region>\w+))?")]
    private static partial Regex SignalRefFallbackPattern();
    ...
    // Limiter reaction: CODE.subcode?[state]? @REGION?
    // Matches: "DA.synthesis @VTA", "5HT.release", "eCB.ANA[↓] @PFC", "GABA.synthesis @PFC"
    [GeneratedRegex(@"^(?<code>\w+)(?:\.(?<subcode>\w+))?(?:\[[^\]]*\])?\s*(?:@(?<region>\w+))?")]
    private static partial Regex ReactionSignalRefPattern();
