...
{
    // 27 population-level optimal ranges extracted from agent prompt dose-response data.
    // 10 chemicals have explicit numeric ranges in the prompts; 17 are derived from
    ...
    public static readonly IReadOnlyDictionary<string, OptimalRange> PopulationRanges =
        new Dictionary<string, OptimalRange>
        {
            ...
            // Context dependent: with safety more=better; with threat=defensive aggression
            ["oxytocin_h"] = new(0.30f, 0.60f, 0.45f, DoseResponseCurve.ContextDependent),

            // ── Peptide layer (10) ──
            // Structural: high sustained = secure attachment architecture
